...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a MUU address (e.g. %1)").arg("MQiobcHQk26RFqV7yT8PycNHbDTg5rmE9W"));
#endif
...
{
    // return if URI is not valid or is no muusic: URI
...
{
    // Convert muusic:// to muusic:
    //
    //    Cannot handle this later, because muusic:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open muusic.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Muusic.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Muusic.lnk
...
{
    return GetAutostartDir() / "muusic.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a muusic.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Muusic\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the muusic app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add muusic app to startup item list
