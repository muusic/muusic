...
// Copyright (c) 2017-2018 The Muusic Team
...
#if defined(NDEBUG)
#error "Muusic cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in umuu) are considered zero fee (for relaying and mining)
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    ret = blockValue * 0.30; // 30% of block reward
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // MUU: We use certain sporks during IBD, so check to see if they are
