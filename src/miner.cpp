...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("MuusicMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("MuusicMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("MuusicMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("muusic-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running MuusicMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
